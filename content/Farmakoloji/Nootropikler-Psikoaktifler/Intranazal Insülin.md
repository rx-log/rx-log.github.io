Kan glikoz ve insülin seviyelerini kayda değer miktarda etkilememektedir<sup>(birkaç çalışma spesifik dozlara bağlı olarak etkilediğini raporlarken, diğer çalışmalar ise bu dozlardan daha düşük dozlarda bile etkilediğini raporlamaktadır)</sup>.

<br>
<h1 style="color:lightgreen;">İnsan</h1>

- Kelime hatırlama testinde<sup>(bir çeşit hafıza testi)</sup> performansı artırmıştır. <a href="https://pubmed.ncbi.nlm.nih.gov/16936707/">R</a>

- Koku ipuculu mekânsal belleğin gecikmeli hatırlanmasını geliştirmiştir. Plazma glikoz seviyeleri değişmemiştir. <a href="https://pubmed.ncbi.nlm.nih.gov/25337926/">R</a>

- Oksipital gri madde aracılığıyla perfüzyonu ve talamusta perfüzyonu artırmıştır<sup>(daha çok yaşlı katılımcılarda)</sup>. Yani serebral kan akışını artırmıştır. <a href="https://pubmed.ncbi.nlm.nih.gov/28291957/">R</a>

- Hipotalamik aktiviteyle ilişkilendirildiği şekilde, tüm vücut insülin hassasiyetini artırmıştır<sup>(daha çok obez olmayanlarda)</sup>. <a href="https://pubmed.ncbi.nlm.nih.gov/25028522/">R</a>

- Doza bağlı olarak bölgesel beyin aktivitesini modüle etmiş<sup>(amigdala, hipotalamus, kaudat, üst frontal girus)?</sup>, ve nabız değişkenliğinin yüksek-frekans alanını normalleştirmiştir. 160UI en güçlü etkilere sahipti, ve bu dozda kan dolaşımına insülin "taşması" tespit edildi<sup>/?\</sup>. <a href="https://pubmed.ncbi.nlm.nih.gov/29095982/">R</a>


<br>
<h1 style="color:lightgreen;">Hayvan/Laboratuvar Ortamı</h1>

- Bir nörotoksinin neden olduğu motor bozukluklarını ve dopamin nöronlarının kaybını<sup>(bunu substantia nigra bölgesinde yapar, sıklıkla dopamin üretimini sağlayan kritik bir beyin bölgesi olarak tanımlanır)</sup> kayda değer miktarda azaltmasına ek olarak, dopamin nöronlarının korunmasını<sup>(substantia nigra bölgesinde)</sup> sağlamıştır. Tirozin hidroksilaz-pozitif nöronlarını ~%74.8 artırmıştır. <a href="https://pubmed.ncbi.nlm.nih.gov/26777890/">R</a>

- Bilişsel disfonksiyon modelinde; öğrenme ve hafıza fonksiyonları kayda değer biçimde gelişmiş/geri kazanılmış, hipokampusta nörojenez iyileşmiş, beyindeki insülin seviyeleri artmış, ve IRS-1-PI3K-Akt-GSK3β insülin sinyal yolunun<sup>(insülinin hücrelerdeki sinyalleme zinciridir)</sup> aktivasyonu bariz şekilde artmıştır<sup>(ancak Ras-Raf-MEK-MAPK yolunun değil)</sup>. Beyin insülin direncini tersine çevirmiştir. <a href="https://pubmed.ncbi.nlm.nih.gov/32399074/">R</a>

- Hafızayı; daha genç modellerde görünen seviyeye benzer olacak şekilde iyileştirmiştir. Hiperpolarizasyon sonrası potansiyeli<sup>(APH, zarın kısa süreliğine dinlenik potansiyelin altına inmesi)</sup> azaltmıştır<sup>(sonuç olarak da aksiyon potansiyeli ateşleme sıklığı artabilir)?</sup>. <a href="https://doi.org/10.1093/gerona/glu314">R</a>

- Kv1.3'ün<sup>(bir voltaj kapılı potasyum kanalıdır; zar potansiyelini, hücresel aktivasyonu ve hücre ölümünü kontrol eder)</sup> bariz fosforilasyonunu uyarmış, insülin reseptör ve postsinaptik yoğunluk 95<sup>(uyarıcı sinapslarda nörotransmiter reseptörleri organize ve stabilize eden bir kılıf proteindir)</sup> ile kanal protein-protein etkileşimlerini artırmıştır. Kısa ve uzun vadeli obje hafıza tanınırlığını, anksiyolitik davranışı, ve koku ayrımını artırmıştır. <a href="https://pubmed.ncbi.nlm.nih.gov/19458242/">R</a>

<h1 style="color:lightgreen;">Deneyimler/Anekdot</h1>

- <b style="color:lightpink;">RX:</b> 

- <b style="color:lightpink;">X:</b> 

- <b style="color:lightpink;">Y:</b> 

- <b style="color:lightpink;">Z:</b> 
